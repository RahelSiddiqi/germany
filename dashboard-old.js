...
function displayIELTSPlan() {
	...
	const renderTask = (id, time, task, duration) => {
		...
		const completedStyle = isChecked
		...
		return `
            <tr class="${completedClass}" data-task-id="${id}" ${completedStyle}>
                <td><input type="checkbox" class="task-checkbox" ${checked} onchange="toggleIELTSTask('${id}', this)"></td>
                <td class="time-cell">${time}</td>
                <td class="task-text" ${textStyle}>${task}</td>
...
function toggleIELTSTask(id, checkbox) {
	...
	// Use checkbox state directly for reliability
